    ...
    return Material(
      ...
      child: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          Row(children: [
            Expanded(child: _select<int>(label: '数据位', value: _dataBits, items: [5, 6, 7, 8].map((v) => DropdownMenuItem(value: v, child: Text('$v'))).toList(), onChanged: (v) => setState(() => _dataBits = v!))),
            const SizedBox(width: 8),
            Expanded(child: _select<int>(label: '校验位', value: _parity, items: const [DropdownMenuItem(value: SerialPortParity.none, child: Text('无')), DropdownMenuItem(value: SerialPortParity.even, child: Text('偶校验')), DropdownMenuItem(value: SerialPortParity.odd, child: Text('奇校验')), DropdownMenuItem(value: SerialPortParity.mark, child: Text('Mark')), DropdownMenuItem(value: SerialPortParity.space, child: Text('Space'))], onChanged: (v) => setState(() => _parity = v!))),
          ]),
          const SizedBox(height: 10),
          Row(children: [
            Expanded(child: _select<int>(label: '停止位', value: _stopBits, items: const [DropdownMenuItem(value: 1, child: Text('1')), DropdownMenuItem(value: 2, child: Text('2'))], onChanged: (v) => setState(() => _stopBits = v!))),
            const SizedBox(width: 8),
            Expanded(child: _select<int>(label: '流控', value: _flowControl, items: const [DropdownMenuItem(value: SerialPortFlowControl.none, child: Text('无')), DropdownMenuItem(value: SerialPortFlowControl.rtsCts, child: Text('RTS/CTS')), DropdownMenuItem(value: SerialPortFlowControl.dtrDsr, child: Text('DTR/DSR')), DropdownMenuItem(value: SerialPortFlowControl.xonXoff, child: Text('XON/XOFF'))], onChanged: (v) => setState(() => _flowControl = v!))),
          ]),
          const SizedBox(height: 20),
